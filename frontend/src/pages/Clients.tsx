...
// import { api } from '../api.ts'; // Usando 'api' conforme seu código
// import { Header } from '../lib/components/Header';
// import { Button } from '../lib/components/Button'; // Certifique-se de importar Button
// import { Modal } from '../lib/components/Modal'; // Certifique-se de importar Modal
// import { ClientForm } from '../lib/components/ClientForm'; // Certifique-se de importar ClientForm
// import { Pagination } from '../lib/components/Pagination'; // Certifique-se de importar Pagination
// import { BiRefresh } from 'react-icons/bi';

// interface Client {
// 	id?: string; // Opcional para cards em criação
// 	name: string;
// 	salary?: string; // Make salary optional
// 	company?: string; // Make company optional
// 	email: string;
// 	status?: 'creating' | 'created' | 'failed'; // Status do card
// }

// export function Clients() {
// 	const [clients, setClients] = useState<Client[]>([]);
// 	const [loading, setLoading] = useState(true); // Descomentado
// 	const [error, setError] = useState<string | null>(null); // Descomentado
// 	const [showCreateModal, setShowCreateModal] = useState(false); // Descomentado
// 	const [currentPage, setCurrentPage] = useState(1); // Mantenha setCurrentPage se for usar a paginação
// 	const [clientsPerPage, setClientsPerPage] = useState(16); // Mantenha setClientsPerPage
// 	const [totalClients, setTotalClients] = useState(0); // Descomentado
...
// 		// É uma boa prática usar uma variável de ambiente para a base URL
// 		const sseUrl = `${
...
// 					// Tenta encontrar e atualizar o card 'creating'
// 					const existingClientIndex = prevClients.findIndex(
// 						// Usamos o email como chave de correspondência para o card temporário
// 						(c) => c.email === newClientData.email && c.status === 'creating',
// 					);

// 					if (existingClientIndex !== -1) {
// 						// Atualiza o card existente com o ID real e status 'created'
...
// 					} else {
// 						// Se não encontrou um card 'creating' correspondente (ex: refresh da página),
// 						// apenas adiciona o novo cliente.
// 						// Verifica se o cliente já existe para evitar duplicatas em caso de refetch
// 						if (!prevClients.some((c) => c.id === newClientData.id)) {
// 							return [{ ...newClientData, status: 'created' }, ...prevClients]; // Adiciona no topo
// 						}
// 						return prevClients;
// 					}
// 				});
// 				setTotalClients((prevTotal) => prevTotal + 1); // Incrementa o total
// 			} catch (e) {
...
// 			eventSourceRef.current?.close(); // Fecha a conexão em caso de erro
...
// 		};
// 	}, [currentPage, clientsPerPage]); // Adicionado currentPage e clientsPerPage como dependências
...
// 			);
// 			// Mapeia os clientes carregados para ter status 'created' por padrão
// 			setClients(
// 				response.data.data.map((c: Client) => ({ ...c, status: 'created' })),
// 			);
// 			setTotalClients(response.data.total); // Atualiza o total
// 		} catch (err) {
...
// 		company?: string; // Make company optional
// 	}) => {
// 		const tempId = `temp-${crypto.randomUUID()}`; // Usando crypto.randomUUID() como você já estava
...
// 		setClients((prevClients) => [tempClient, ...prevClients]); // Adiciona o card temporário imediatamente
// 		setShowCreateModal(false); // Fecha o modal após a submissão

// 		try {
// 			await api.post('/clients', clientData);
// 			// A atualização final do card virá via SSE
// 		} catch (err: any) {
// 			console.error(
...
// 			// Em caso de falha na requisição API, atualiza o card temporário para 'failed'
// 			setClients((prevClients) =>
// 				prevClients.map((client) =>
// 					client.id === tempId ? { ...client, status: 'failed' } : client,
// 				),
// 			);
// 			// Você pode adicionar um timeout aqui para remover cards falhos após um tempo, se desejar
// 		}
// 	};

// 	// --- Funções de callback para ClientCard (preenchidas com console.log para exemplo) ---
// 	const handleAddClientToSelection = (clientId: string) => {
// 		console.log(`Adicionar cliente ${clientId} à seleção.`);
// 		// Implemente sua lógica para adicionar à seleção
// 	};

// 	const handleEditClient = (clientId: string) => {
// 		console.log(`Editar cliente ${clientId}.`);
// 		// Implemente sua lógica para edição (abrir modal de edição, carregar dados, etc.)
// 		setShowCreateModal(true); // Abre o modal de criação/edição
// 		// Você pode passar os dados do cliente para o modal se necessário
// 		setEditClientData(clients.find((c) => c.id === clientId));
...
// 						<select
// 							className="bg-gray-200 border border-gray-200 rounded-md py-1 px-2 text-sm focus:outline-none focus:ring-1 focus:ring-[#FF6B00]"
...
// 								className="inline-block ml-2 text-xl text-red-500"
// 							/>
// 						</span>
// 					</div>
// 				)}
...
// 							{clients.map((client) => (
// 								<ClientCard
// 									key={client.id || client.email} // Use o ID real ou o email como fallback para a key
// 									id={client.id} // Passe o ID
// 									name={client.name}
// 									salary={client.salary?.toString() || ''}
// 									company={client.company?.toString() || ''}
// 									status={client.status} // Passa o status
// 									onAdd={() =>
// 										client.id && handleAddClientToSelection(client.id)
// 									}
// 									onEdit={() => client.id && handleEditClient(client.id)}
// 									onDelete={() => client.id && handleDeleteClient(client.id)}
// 								/>
// 							))}
// 						</div>

// 						<div className="text-center mb-8">
// 							<div className="text-center mb-8 position-fixed bottom-0">
// 								<Button
// 									onClick={() => setShowCreateModal(true)}
// 									className="bg-[#FF6B00] hover:bg-orange-700 text-white px-8 py-3 text-lg"
// 								>
// 									Criar cliente
// 								</Button>
// 							</div>
